    ...
    <div class="container">
      ...
      <div class="py-5">
        <div class="container">
          <div class="row">
            <div class="col-md-12">
              <h1 class="text-center display-3 text-primary">REQUESTS</h1>
            </div>
          </div>
        </div>
      </div>
      ...
      //$profSubCategoryID = 1;
                $sql1 = "SELECT professional.subcategory_id FROM professional,user WHERE user.id = professional.user_id && user.username='$usernameForRequestList'";
      ...
      //taking subcategoryID of current user which is proffesional
                ...
                $subCatID = $result1['subcategory_id'];
                
                
                $sqlForCity = "SELECT user.city_id FROM professional,user WHERE user.id = professional.user_id && user.username='$usernameForRequestList'";
      //taking subcategoryID of current user which is proffesional
                $consIdTable1=mysqli_query($db, $sqlForCity);
                $result1 = mysqli_fetch_assoc($consIdTable1);
                $cityID = $result1['city_id'];
                
                
                $sql2 = "SELECT consumer.name, consumer.surname, request.time_interval,request.information, request.request_id FROM user,request,consumer WHERE request.consumer_id = consumer.consumer_id && consumer.consumer_id= user.id && user.city_id = '$cityID' && request.subcategory_id = '$subCatID' && request.isTaken = '0'";
                $consIdTable2 = mysqli_query($db, $sql2);
      while($row = mysqli_fetch_assoc($consIdTable2)){   //Creates a loop to loop through results
        //echo "dsadas";
        echo "<tr><td>" . $row['name'] . "</td><td>" . $row['surname'] . "</td><td>" . $row['time_interval'] . "</td><td>" . $row['information'] . "</td><td><a class='btn btn-primary' style='text-decoration:  none;' href='giveOffer.php?reqId=".$row['request_id']."'>Give Offer</a></td></tr>";  //$row['index'] the index here is a field name
      }
